...
```{r running a different xgb}
cleanPersonDataset_model3 <- cleanPersonDataset %>%
  mutate(TwoMonthsWithoutActivity = factor(cleanPersonDataset$TwoMonthsWithoutActivity)) %>%
  mutate(MaritalStatusValueId = as.factor(cleanPersonDataset$MaritalStatusValueId)) %>%
  mutate(IsImported = as.factor(IsImported)) %>%
  mutate(IsImported = recode(IsImported, "0" = "NotImported", "1" = "Imported")) %>%
  mutate(AgeClassification = recode(AgeClassification, "0" = "Unknown", "1" = "Adult", "2" = "Child")) %>%
  drop_na()



set.seed(987)
PersonDataset_split_model3 <- initial_split(cleanPersonDataset_model3, prop = 0.80, strata = TwoMonthsWithoutActivity)
PersonDataset_training_model3 <- PersonDataset_split_model3 %>% training()
PersonDataset_test_model3 <- PersonDataset_split_model3 %>% testing()

caretModel <- train(
  TwoMonthsWithoutActivity ~ ., 
  data = PersonDataset_training_model3, 
  method = 'xgbTree', 
...
PersonDataset_test_model2 <- PersonDataset_split_model2 %>% 
  testing() %>%
  select(-Gender) %>%
  mutate(IsImported = as.factor(IsImported)) %>%
  mutate(AgeClassification = as.factor(AgeClassification)) %>%
  mutate(TwoMonthsWithoutActivity = as.factor(TwoMonthsWithoutActivity))

levels(PersonDataset_test_model2$IsImported) <- c("Imported", "NotIMported")
levels(PersonDataset_test_model2$AgeClassification) <- c("Adult", "Child")
levels(PersonDataset_test_model2$TwoMonthsWithoutActivity) <- c("Stay", "Leave")


confusionMatrix(predict(caretModel, PersonDataset_test_model2), PersonDataset_test_model2$TwoMonthsWithoutActivity)
...
TODO: 
  ...
  * Recommendation
  * Measure impact after applying
  * Ensure to communicate the model was trained for my church
