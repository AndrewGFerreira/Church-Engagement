...
```{r Loading motivation echo=FALSE, out.width='100%'}
knitr::include_graphics('resources/motivation.png')
```
...
personDataset <- read.csv("datasets/Experiment4.csv", header = TRUE)
...
```{r Investigating factors , message=FALSE, warning=FALSE}
library(tidyr)
library(tidymodels)
library(rpart)
library(rpart.plot)
...
personDataset %>%
  select(-Id, -Baptism, -CommitToChrist, -IAnNew, -PrayerRequest, -RenewCommitToChrist, -X3MonthsTitheChallenge) %>%
...
cleanPersonDataset <- personDataset %>%
  select(-Id, -Baptism, -CommitToChrist, -IAnNew, -PrayerRequest, -RenewCommitToChrist, -X3MonthsTitheChallenge, -ActivationDate, -PersonCreatedDate, -FirstAttendanceDate, -LastAttendanceDate, -FirstGift, -LastGift, -MonthFirstAttendance, -MonthLastAttendance, ) %>%
  mutate(Age = ifelse(is.na(Age), 0, Age)) %>%
  ##filter(IsImported == 0) %>%
  filter(TimeToActivate > 0) %>%
  filter(!Age %in% outliersAge) %>%
  ...
  filter(!TimeToActivate %in% outliersTimeToActivate) %>%
  filter(!MonthsBetweenFirstAndLastAttendance %in% outliersMonthsBetweenFirstAndLastAttendance) %>%
  mutate(TwoMonthsWihtoutActivity = as.factor(TwoMonthsWihtoutActivity)) %>%
  mutate(TwoMonthsWihtoutActivity = str_replace(TwoMonthsWihtoutActivity, "1", "LeaveTheChurch")) %>%
  mutate(TwoMonthsWihtoutActivity = str_replace(TwoMonthsWihtoutActivity, "0", "StayAtChurch")) %>%
...
summary(cleanPersonDataset)

set.seed(987)

PersonDataset_split <- initial_split(cleanPersonDataset, prop = 0.80, strata = TwoMonthsWihtoutActivity)
PersonDataset_training <- PersonDataset_split %>% training()
PersonDataset_test <- PersonDataset_split %>% testing()


tree_model = rpart(TwoMonthsWihtoutActivity ~ ., data = PersonDataset_training, method="class", minsplit = 10, minbucket=3) 
summary(tree_model)

rpart.plot(tree_model)

test_tree_model <- predict(tree_model, PersonDataset_test, type = 'class')
table_mat <- table(PersonDataset_test$TwoMonthsWihtoutActivity, test_tree_model)
